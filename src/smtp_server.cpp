...
void SmtpServer::send_message(const std::string & msg, bool is_split, bool is_ssl) const {
    ...
    do {
        ...
        if ((sent = is_ssl ? SSL_write(c_ssl, buf.c_str(), buf.size()) : send(fd, buf.c_str(), buf.size(), 0)) == -1) {
            throw SendingError {};
        }
        bytes_sent += sent;
...
void SmtpServer::login(const std::string & username, const std::string & password) {
    ...
    send_message(Base64::base64_encode(reinterpret_cast<const unsigned char *>(username.c_str()), username.length()),
                 false, true);
    if (!check_status_code(receive_message(true).c_str(), status_codes::AUTH))
        throw AuthError {AuthErrorType::INVALID_USERNAME};
    send_message(Base64::base64_encode(reinterpret_cast<const unsigned char *>(password.c_str()), password.length()),
                 false, true);
